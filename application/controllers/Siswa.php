...
class Siswa extends CI_Controller
{
    ...
    {
        $data['data'] = $this->db->get_where('transaksi ', ['id' => $id])->row();
        $c['data'] = $this->db->get_where('data_siswa', ['id' => $id])->row();
    ...
    {
        $data['siswa'] = $this->Siswas_model->tampildata()->result();
        $data['transaksi'] = $this->Siswas_model->getAllTransaksi();
        $data['kelas'] = $this->Siswas_model->getAllKelas();
        $this->load->view('print_siswa_all', $data);
    }

    public function search()
    {
        $keyword = $this->input->post('keyword');
        $data['siswa'] = $this->Siswas_model->get_keyword($keyword);
        $this->load->view('template_admin/header', $data);
        $this->load->view('template_admin/sidebar', $data);
        $this->load->view('siswa/datasiswa', $data);
        $this->load->view('template_admin/footer', $data);
